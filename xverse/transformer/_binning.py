...
class MonotonicBinning(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        try:
            fit_X = pd.DataFrame(check_array(temp_X, accept_sparse=True, force_all_finite=False, copy=True), columns=fit_on_features)
        except:
            raise ValueError("The input feature(s) should be numeric type. Some of the input features has character values in it. Please use a encoder before performing monotonic operations.")
        
        #apply the monotonic train function on dataset
        fit_X.apply(lambda x: self.train(x, y), axis=0)
        return self
    
    #check the cardinality of each column present in input data. Input data X is a Pandas Series type. If the cardinality is less than user provided input, then the input data will be processed as is. No fit will be performed on the data. If the cardinality is greater, fit will be performed. 
    ...
    #monotonic binning - The function is applied on each columns identified in the fit function. Here, the input X is a Pandas Series type.
    def train(self, X, y):
        ...
        if fitted: #fit is required
            ...
            """Calculate spearman correlation for the distribution identified. If the distribution is not monotonic, reduce bins and reiterate. Proceed until either one of the following happens,
            ...
            """
            Execute this block when monotonic relationship is not identified by spearman technique. We still want our code to produce bins.
            """
